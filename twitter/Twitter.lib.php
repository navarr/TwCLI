...
class Twitter
{
	...
	public function require_login($return_to = NULL)
	{
		...
		switch($this->getVar("OAUTH_STATE"))
		{
			...
			default:
				...
				$tok = $this->auth->getRequestToken(/*$this->getVar("RETURN_URI")*/);
	...
	public function credential_login($username,$password)
	{
		$this->setAuth(new TwitterOAuth($this->key, $this->secret));
		$tok = $this->auth->getXAuthToken($username,$password);
		$this->setVar("OAUTH_ACCESS_TOKEN", $tok["oauth_token"]);
		$this->setVar("OAUTH_ACCESS_TOKEN_SECRET", $tok["oauth_token_secret"]);
		$this->setAuth(new TwitterOAuth($this->key, $this->secret, $this->getVar("OAUTH_ACCESS_TOKEN"), $this->getVar("OAUTH_ACCESS_TOKEN_SECRET")));
		$this->completeAuth();
	...
	{
		if (!$this->hasVar("QUICKDATA") || $verify_credentials || (isset($data["id"]) && $data["id"] > 0))
		{
			$this->forceauth = TRUE;
	...
	public function get_sn_from_id($id)
	{
		...
		else
		{
			...
			else
			{
				...
				$this->cache->user_store($t["result"]);
				$t = $t["result"];
			}
...
class TwitterCache
{
	...
	{ // $data should be an array of the json_decoded api->users_show
		if(isset($data["TAPI"])) { $data = $data["result"]; }
	...
	{ // Use Types TWITTER_UID and TWITTER_SN
		...
		{
			if (isset($cache["BY_SN"][strtoupper($user)]) && isset($cache["BY_ID"][$cache["BY_SN"][strtoupper($user)]]["id"]))
			{
				return TRUE;
			}
			else
...
class TwitterAPI
{
	...
	public $noauth = FALSE;
	public $contribute_to = NULL;
	public $contributing = FALSE;
	...
	// Setup Contributions
	public function contribute_to($id = null)
	{
		if(!$id) { return $this->contribute_to; }
		else { $this->contribute_to = $id;$this->contributing = TRUE;return $id; }
	}
	public function contributing($status = null)
	{
		if($status !== TRUE && $status !== FALSE) { return $this->contributing; }
		else { $this->contributing = $status;return $status; }
	}
	...
	public function statuses_update($status, $reply_to = null, $lat = null, $long = null, $place_id = null, $display_coordinates = null)
	{
		$vars = array ();
		$vars["status"] = $status;
		if ($reply_to)
		...
		{
			$vars["display_coordinates"] = $display_coordinates;
		}
	...
	{ // "user1", TWITTER_SN, 152352, TWITTER_ID, "user3", "user4", 3252352 - Will attempt to auto-identify if its not set.
		$sns = array();
		$ids = array();
		for($i = 0; $i < func_num_args(); $i++)
		{
			if($i+1 != func_num_args() && (func_get_arg($i+1) === TWITTER_SN || func_get_arg($i+1) === TWITTER_ID))
			{
				if(func_get_arg($i+1) === TWITTER_SN) { $sns[] = func_get_arg($i); }
				if(func_get_arg($i+1) === TWITTER_ID) { $ids[] = func_get_arg($i); }
				$i++;
			} else {
				if(!is_string(func_get_arg($i))) { $ids[] = func_get_arg($i); }
				else { $sns[] = func_get_arg($i); }
			}
		}
		$vars = array();
		if(count($sns) > 0) { $vars["screen_name"] = implode(",",$sns); }
		if(count($ids) > 0) { $vars["user_id"] = implode(",",$ids); }
		$r = $this->callMethod("users/lookup",$vars);
	...
	public function statuses_friends($u, $type = TWITTER_ID, $cursor = null)
	{
		...
		foreach($json["result"] as $userData)
		{
	...
	{
		return $this->callMethod($this->t->screen_name."/lists/".$name, null, "DELETE");
	}
	...
	public function account_rateLimitStatus()
	{
		...
		$this->t->setRateLimit($json["result"]["hourly_limit"],$json["result"]["remaining_hits"],$json["result"]["reset_time_in_seconds"]);
		return $json;
	}
	...
	// Geo Methods
	public function geo_id($id)
	{
		return $this->callMethod("geo/id/$id");
	}
	public function geo_reverseGeocode($lat,$long,$accuracy = null,$granularity = null,$max_results = null)
	{
		$vars = array("lat" => $lat, "long" => $long);
		if($accuracy) { $vars["accuracy"] = $accuracy; }
		if($granularity) { $vars["granularity"] = $granularity; }
		if($max_results) { $vars["max_results"] = $max_results; }
		return $this->callMethod("geo/reverse_geocode",$vars);
	}
	...
	protected function callMethod($method, $vars = null, $type = null)
	{
		...
		if($this->contributing && $this->contribute_to)
		{
			$vars["contributingto"] = $this->contribute_to;
		}
	...
	//	if(is_string($result)) { $tResult = $result; $result = array(); $result["string"] = $tResult; }
		$var = array();
		$var["result"] = $result;
		$this->call_log[] = $uri;
		$var["TAPI"]["response_code"] = $http_code;
		return $var;
	}
...
// Definitions
	...
	define("TAPI_IDENTIFIER", "simpleTAPI v0.3.5");
